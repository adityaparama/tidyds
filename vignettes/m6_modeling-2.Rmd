...
sherlock_tidy <- sherlock %>% 
  filter(!is.na(text)) %>% 
  group_by(story) %>%
  unnest_tokens(word, text) %>% #mengubah kalimat menjadi kata
  ungroup() %>% #setelah melakukan groupping & diproses, lebih baik langsung di unroup
  anti_join(stop_words) #data set, daftar kata-kata umum yang digunakan. Biasanya tidak perlu ditext analysis
...
```{r}
sherlock_tfidf %>% 
  filter(story == "ADVENTURE III. A CASE OF IDENTITY") %>% 
  top_n(15, tf_idf) %>% 
  ggplot(aes(x = reorder(word, tf_idf), y = tf_idf)) +
  geom_col() +
  coord_flip() +
  labs(
    ...
    y = "tf-idf",
    title = "Kata terpenting pada cerita Sherlock Holmes",
    subtitle = "Subcerita 'Adventure III. A Case of Identity"
  ) +
  theme_light()
```
...
sherlock_beta %>% 
  ...
  labs(
    x = "",
    y = "beta",
    title = "Peluang Kemunculan Kata pada cerita Sherlock Holmes",
    subtitle = "Jumlah Topic = 1"
  ) +
  theme_light()
```
